...
## SCA · estudantes (26/09/2026)
Pedido do Dilson: versão para estudantes de medicina com conduta, doses e escores; a Iris explica o que é a SCA e depois só a voz dela sobre uma animação do **2Doctor em inglês** pesquisando como conduzir a SCA.
- 2Doctor = `scripts/wmed-app` (branch `codex/2doctor-preview-20260925`, worktree `_worktrees/2doctor-preview`, publicado em www.2doctor.ai/2doctor/). A animação (`app-motion/index.html` + `conteudo.js`, render em `runway/render-app.mjs`) reproduz o app: tokens de `src/doctor/doctor.css` (tinta #142d40, verde #005d5b, balão #edf4f2), fonte Manrope (OFL, copiada para `app-motion/fonts/`), monograma de `Navigation.jsx`, tela "How can I help today?" com abas Automatic / Quick reference / Learn. A pergunta real no app exige login (conta Vytal Acadêmico): a resposta mostrada é nossa, conferida nas diretrizes, no estilo do app.
- Conferência (agente, 26/09; ESC 2023 em PDF integral, páginas do PDF advance article; ACC/AHA 2025 texto integral; Six 2008 no PMC):
  - ECG < 10 min do primeiro contato (I B, Rec. Table 1, p.19); hs-cTn 0h/1h (melhor) ou 0h/2h (p.20); O2 só se SaO2 < 90% (I C), O2 de rotina III (Rec. Table 3, p.26).
  - ICP primária se diagnóstico→ICP < 120 min (I A, Rec. Table 4, p.31); fibrinólise iniciada em até 10 min do diagnóstico (texto §5.3.1.1, p.30); angiografia 2–24 h após o bolus lítico (p.27/30).
  - AAS 150–300 mg VO (75–250 IV), manutenção 75–100 mg/dia (I A, Rec. Table 5, p.39). Prasugrel 60 → 10 mg (5 mg se ≥ 75 a ou < 60 kg) (I B); preferível ao ticagrelor se vai para ICP (IIa B); AVC prévio contraindica (Tabela 6, p.32; AIT não achado na ESC). Ticagrelor 180 → 90 mg 2x/dia (I B). Clopidogrel 300–600 → 75 mg (I C) só se os outros indisponíveis/contraindicados. Pré-tratamento de rotina não recomendado na SCASSST com invasivo < 24 h (III A); pode ser considerado no IAMCSST com ICP primária (IIb B). HNF 70–100 UI/kg IV na ICP (I C). DAPT 12 meses salvo alto risco de sangramento (I A).
  - Invasivo imediato < 2 h: instabilidade/choque, dor refratária, arritmia grave, complicação mecânica, IC aguda, ST-T dinâmico recorrente (I C). Precoce < 24 h **IIa A** (era I em 2020): NSTEMI confirmado, ST-T dinâmico, supra transitório, GRACE > 140.
  - Escores: a ESC recomenda escore estabelecido "e.g. GRACE" (IIa B); o HEART não está nas recomendações da ESC (fonte: Six 2008: 5 itens 0–2; faixas 0–3 / 4–6 / 7–10; n = 120, não citar taxas). TIMI 7 variáveis (ACC/AHA 2025 Tabela 5). Killip I–IV (fonte secundária; original Killip & Kimball 1967).

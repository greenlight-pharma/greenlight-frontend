...
// Parâmetros extras da cena: QS="mode=dpoc&x=1" node stills.mjs ...
...
const p = await b.newPage(); p.on('pageerror', e => { console.log('[erro]', e.message); b.close().then(() => process.exit(1)); }); p.on('console', m => /error|warn/i.test(m.type()) && console.log('[console]', m.text().slice(0, 200)));
await p.setViewport({ width: +w, height: +h });
await p.goto(`http://127.0.0.1:3070/wmed-video-studio/models3d/view.html?scene=${scene}&w=${w}&h=${h}${process.env.QS ? '&' + process.env.QS : ''}`);

...
export default function PrimeiroCuidado(){
 ...
 return <div className="page narrow care-onboarding"><span className="eyebrow">VAMOS COMEÇAR</span><h1>Quem vai receber o cuidado?</h1><p>Os lembretes chegam pelo WhatsApp.</p><div className="grid2"><Surface><Icon name="user" size={32}/><h2>Para mim</h2><Button className="big" onClick={()=>setChoice(true)}>Cuidar de mim</Button></Surface><Surface><Icon name="heart" size={32}/><h2>Para um familiar</h2><Button className="big" onClick={()=>setChoice(false)}>Cuidar de um familiar</Button></Surface></div><p className="muted small">Depois, informe as condições de saúde.</p>{choice!==null&&<NovaPessoa paraMim={choice} onboarding onClose={()=>setChoice(null)} onSaved={async phone=>{await app.reload();navigate(`/programas/${phone}?inicio=1`);}}/>}</div>;
}

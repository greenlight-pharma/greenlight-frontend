...
export default function NovaPessoa({ onClose, paraMim: inicial }: { onClose: () => void; paraMim?: boolean }) {
  ...
  const concluir = () => { onClose(); if (gestacao) navigate(`/p/${withCountry(phone)}/gestacao`); };
  ...
  async function salvar() {
    ...
    try {
      ...
      if (aviso) setDone(aviso.replace(/paciente/g, "pessoa")); else concluir();
    } catch (e) {
  ...
  return <Dialog title="Quem vai receber os lembretes?" onClose={onClose} locked={busy}>
    ...
    {paraMim !== null && <>
      ...
      <label className="row"><input type="checkbox" checked={gestacao} onChange={e=>setGestacao(e.target.checked)}/><span>Configurar o programa Gestação depois do cadastro</span></label>

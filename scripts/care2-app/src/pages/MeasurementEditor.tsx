...
export default function MeasurementEditor({ patient, onClose }: { patient: Patient; onClose: (saved: boolean) => void }) {
  const [delivery, setDelivery] = useState<boolean | null>(null);
  const [type, setType] = useState("pressao");
  const [times, setTimes] = useState<string[]>([]);
  const [duration, setDuration] = useState(""), [days, setDays] = useState(""), [endDate, setEndDate] = useState("");
  ...
  async function save() {
    if (busy) return;
    if (!times.length || !times.every(Schedule.valid) || new Set(times).size !== times.length) return setError("Escolha quantas vezes por dia e confira os horários.");
    let end = "";
  ...
  return <Dialog wide title="Pedir aferição" eyebrow={patient.name} onClose={() => onClose(false)} locked={busy}>
    ...
    <Notice>{error}</Notice>
    <Button className="big" icon="send" busy={busy} onClick={save}>Pedir aferição</Button>
  </Dialog>;
}

...
export default function Genetics(){
 ...
 <div className="gene-viewport" ref={viewport}>
 ...
 <div className="gene-controls"><span>Arraste para girar</span><div><Present stageRef={viewport} name={`wmed-genetica-${stage.id}`}/><button onClick={()=>setSpin(!spin)} aria-label={spin?'Pausar giro':'Girar modelo'} aria-pressed={spin}>{spin?<Pause size={17}/>:<Play size={17}/>}</button><button aria-label="Aproximar modelo" onClick={()=>handle.current?.zoom?.(.85)}><Plus size={18}/></button><button aria-label="Afastar modelo" onClick={()=>handle.current?.zoom?.(1.18)}><Minus size={18}/></button><button aria-label="Restaurar enquadramento" onClick={()=>handle.current?.fit?.()}><RotateCcw size={17}/></button></div></div>

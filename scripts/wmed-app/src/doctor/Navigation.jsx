...
export function QuickActions({ navigate, onCase, onPrompt }) {const { t, locale } = useI18n();
  return <div className="doctor-quick-area"><div className="doctor-quick-actions"><button onClick={onCase}><FileText size={20} /><span>{t("Discutir um caso")}<small>{t("Organize o racioc\xEDnio cl\xEDnico")}</small></span><ArrowUpRight size={17} /></button><button onClick={() => navigate('scores')}><Calculator size={20} /><span>{t("Consultar um score")}<small>{t("Calculadoras \xE0 m\xE3o")}</small></span><ArrowUpRight size={17} /></button><button onClick={() => navigate('questoes')}><GraduationCap size={21} /><span>{t("Estudar para provas")}<small>{t("Quest\xF5es e coment\xE1rios")}</small></span><ArrowUpRight size={17} /></button></div><div className="doctor-suggestions"><span>{t("Experimente")}</span>{[t("Compare asma e DPOC"), t("Explique o ciclo card\xEDaco")].map(t).map((text) => <button key={text} onClick={() => onPrompt(text)}>{text}<ArrowUpRight size={13} /></button>)}</div></div>;
}

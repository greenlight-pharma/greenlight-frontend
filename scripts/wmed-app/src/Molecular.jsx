...
import {parseSDF,parsePDB,buildMolecule,ELEMENT_COLORS} from './lumen/molecule';
...
function LumenMolecule({entry,style,hydrogens,spin,onAtom,onReady,retry,handle}){
 ...
 useEffect(()=>{if(!text)return;const node=el.current;let renderer;
  ...
  const built=buildMolecule(mol,{style,hydrogens,theme});scene.add(built.group);
...
export default function Molecular(){
 ...
 <div className="mol-legend" aria-label={style==='cartoon'?'Elementos dos ligantes; fitas coloridas pela sequência':'Legenda dos elementos'}>{style==='cartoon'&&<span>Fitas: sequência · ligantes:</span>}{Object.entries(elements).filter(([s])=>s!=='H'||hydrogens).map(([symbol,[name,color]])=><span key={symbol} title={name}><i style={{background:LUMEN&&style!=='cartoon'?ELEMENT_COLORS[lumenTheme()][symbol]:color}}/>{symbol}</span>)}</div>

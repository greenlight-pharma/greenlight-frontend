...
export function useChatHistory({api,scope,messages,setMessages,busy}){
 ...
 async function call(payload){const r=await fetch(`${api}/history`,{method:'POST',headers:{'Content-Type':'application/json','X-WMed-Request':'1'},body:JSON.stringify(payload)});const d=await r.json();if(!r.ok)throw Error(t(d.error||'Histórico indisponível.'));return d;}

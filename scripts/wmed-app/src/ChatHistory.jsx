...
export function useChatHistory({api,scope,messages,setMessages,busy}){
 ...
 async function call(payload){const r=await fetch(`${api}/history`,{method:'POST',headers:{'Content-Type':'application/json','X-WMed-Request':'1','X-WMed-Lang':lang},body:JSON.stringify(payload)});const d=await r.json();if(!r.ok)throw Error(t(d.error||'Histórico indisponível.'));return d;}
 ...
 useEffect(()=>{
  ...
  s.writer=createHistoryWriter(async(id,rows)=>{if(state.current!==s)throw Error(t('A conta mudou. Entre novamente para salvar.'));const d=await call({action:'save',id,messages:rows});if(state.current===s){try{localStorage.setItem(s.key,d.id);}catch{}}return d;});state.current=s;
  setLoading(true);
...
export default function ChatHistory({history,onClose,onNew,onSelect}){return <div className="modal-shade" onClick={onClose}><section className="modal history-modal" role="dialog" aria-modal="true" aria-labelledby="history-title" onClick={e=>e.stopPropagation()}><button autoFocus className="icon-btn close" aria-label={t("Fechar histórico")} onClick={onClose}><X/></button><h2 id="history-title">{t("Suas conversas")}</h2><p>{t("Chats salvos na sua conta WMed. As 50 conversas mais recentes aparecem aqui.")}</p><button className="history-new" onClick={onNew} disabled={history.loading}><Plus size={17}/> {t("Nova conversa")}</button>{history.error&&<p role="alert" className="error">{history.error}<button onClick={history.refresh}>{t("Tentar novamente")}</button></p>}{history.loading?<p role="status">{t("Carregando…")}</p>:history.items.length?<div className="history-list">{history.items.map(item=><button key={item.id} onClick={()=>onSelect(item.id)}><MessageSquare size={19}/><span><strong>{item.titulo||t('Conversa')}</strong><small>{new Date(item.updatedAt).toLocaleDateString(locale)} · {t('{n} mensagens',{n:item.mensagens})}</small></span></button>)}</div>:<p>{t("Suas próximas conversas aparecerão aqui.")}</p>}</section></div>;}

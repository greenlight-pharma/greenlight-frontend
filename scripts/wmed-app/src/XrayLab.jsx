...
function Anatomy({data,view,selected,onSelect,reset,geometry,probe}){
 ...
 useEffect(()=>{
  ...
  const fit=()=>{if(!el.clientWidth||!el.clientHeight)return;renderer.setSize(el.clientWidth,el.clientHeight);camera.aspect=el.clientWidth/el.clientHeight;camera.fov=latest.current.geometry?43:projectionFov(data,Math.min(1,camera.aspect));camera.updateProjectionMatrix()};
  ...
  api.current={camera,controls,materialList,rig};
  const tick=()=>{frame=requestAnimationFrame(tick);controls.update();renderer.render(scene,camera)};tick();
  return()=>{disposed=true;cancelAnimationFrame(frame);observer.disconnect();controls.dispose();release(root);rig.traverse(o=>{o.geometry?.dispose();o.material?.map?.dispose();o.material?.dispose()});renderer.dispose();renderer.domElement.remove();api.current=null};
 },[data]);
 useEffect(()=>{const a=api.current;if(!a)return;a.controls.enableRotate=!!geometry;a.controls.minDistance=geometry?500:360;a.controls.maxDistance=geometry?4000:1000;
  a.camera.zoom=geometry?1:1.35;a.camera.fov=geometry?43:projectionFov(data,Math.min(1,a.camera.aspect));a.camera.updateProjectionMatrix();
 ...
 useEffect(()=>{const a=api.current;if(!a)return;const g=a.rig;let cancelled=false;
  const clear=()=>{g.traverse(o=>{o.geometry?.dispose();o.material?.map?.dispose();o.material?.dispose()});g.clear()};clear();if(!geometry)return;
...
function Radiograph({data,view,selected,onSelect,overlay,contrast,onProbe,probe}){
 ...
 useEffect(()=>{
  ...
  Promise.all([load(BASE+view.image),load(BASE+view.mask)]).then(([rx,mask])=>{
   ...
   if(overlay!=='off')for(let i=0;i<bitpixels.length;i+=4){const x=(i/4)%data.size,y=Math.floor(i/4/data.size),matches=part?[part]:data.parts;
...
export default function XrayLab(){
 ...
 return <section ref={ref} className="xr-lab" aria-label="Laboratório de radiografia em 3D">
  <header className="xr-header"><div><span className="xr-eyebrow"><FlaskConical size={12}/> LABORATÓRIO DE IDEIAS <b>PROTÓTIPO</b></span><h1>Radiografia em 3D</h1><p>Pelve · Veja a mesma anatomia no 3D e na projeção.</p></div><button aria-label="Sobre o exame" onClick={()=>setInfo(!info)} aria-expanded={info}><Info size={17}/><span>Sobre o exame</span></button></header>

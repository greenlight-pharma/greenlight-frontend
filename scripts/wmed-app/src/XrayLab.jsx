...
function Anatomy({data,view,selected,onSelect,reset,geometry,probe}){
 ...
 useEffect(()=>{for(const mesh of api.current?.materialList||[]){const active=mesh.userData.part===selected;mesh.material.color.set(active?'#62dfd4':data.parts.find(p=>p.id===mesh.userData.part)?.color||'#ded5c0');mesh.material.emissive.set(active?'#104f49':'#000000');mesh.material.emissiveIntensity=.5;const opacity=selected?(active?1:.14):(data.parts.find(p=>p.id===mesh.userData.part)?.opacity??1);if(mesh.material.transparent!==(opacity<1)){mesh.material.transparent=opacity<1;mesh.material.needsUpdate=true;}mesh.material.opacity=opacity;mesh.material.depthWrite=opacity===1}},[selected,data,status]);

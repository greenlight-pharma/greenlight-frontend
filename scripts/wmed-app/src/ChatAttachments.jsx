...
async function prepare(file) {
  ...
  let text;
  if (extension === 'docx') {const module = await import('mammoth/mammoth.browser.js');text = (await (module.default || module).extractRawText({ arrayBuffer: await file.arrayBuffer() })).value;} else
  if (['txt', 'md'].includes(extension)) text = await file.text();else
  throw Error('Use JPG, PNG, WebP, PDF, DOCX, TXT ou MD.');

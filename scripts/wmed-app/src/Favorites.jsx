...
const KINDS={score:msg('Scores'),calc:msg('Calculadoras'),med:msg('Medicações'),cid:msg('Condições'),topic:msg('Resumos'),guide:msg('Guias')};
export default function Favorites({onLogin}){
 ...
 {s.loading?<p role="status">{t('Carregando…')}</p>:rows.length?<div className="resource-grid">{rows.map(r=><a className="resource-card favorite-card" key={r.kind+r.ref} href={r.href}><small>{t(KINDS[r.kind]||r.kind)} {r.favorite&&<Star size={13} fill="currentColor"/>}{r.note&&<StickyNote size={13}/>}</small><h3>{r.title}</h3>{r.note&&<p>{r.note}</p>}<span>{new Date(r.updatedAt).toLocaleDateString(locale)} <ArrowUpRight size={15}/></span></a>)}</div>:<p>{t('Nada salvo ainda. Use a estrela ou "Anotar" em scores, medicações, condições, resumos e guias.')}</p>}</>}
 </section>;
}
